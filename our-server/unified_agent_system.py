...
Please respond in the style of {self.name} with {self.language} language, always addressing as "ভাই"."""
    ...
    def process_message(self, message: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        # Fallback to cloud providers (only if not force local)
        cloud_response = ai_providers.fallback_chain(prompt, context)
        if cloud_response.get("response"):
            verification = self.verify_truth(cloud_response["response"], context)
            
            return {
                "response": cloud_response["response"],
                "agent": self.name,
                "capability": capability,
                "capability_info": self.capabilities.get(capability, {}),
                "truth_verification": verification,
                "source": "cloud_fallback",
                "provider": cloud_response.get("provider"),
                "timestamp": time.time()
            }
