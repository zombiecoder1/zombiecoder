...
def start_interceptor():
    ...
    try:
        # Try to run on all interfaces (for better compatibility)
        app.run(
            host='0.0.0.0',
            port=5000,
            debug=False,
        ...
        logger.error(f"Failed to start on port 5000: {e}")
        try:
            # Fallback to port 8443 if 5000 fails
            app.run(
                host='127.0.0.1',
                port=8443,
                debug=False,
            ...
            logger.error(f"Failed to start on port 8443: {e}")
            # Last resort - try any available port
            app.run(
                host='127.0.0.1',
